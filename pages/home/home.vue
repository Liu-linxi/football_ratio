...
<script>
	...
	export default {
		...
		methods: {
			...
			async initData() {
				const { data } = await match({ page: this.page, oderby: this.oderby }, { token: this.userInfo.token });
				this.page++;
			...
			wsData(data) {
				...
				// data && data?.map(item => {
				// 	this.initList.map((item2, index) => {
				// 		if (item.id === item2.id) {
				// 			this.$set(this.initList, index, item);
				// 		}
				// 	})
				// })
			},
